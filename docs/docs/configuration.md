...
| `caching.inter_query_builtin_value_cache.named.regex.max_num_entries`    | `int`   | No       | Maximum number of entries in the `regex` cache, used by the [`regex` built-in functions](./policy-reference/builtins/regex) to cache compiled patterns. OPA will drop random items from the cache if this limit is exceeded. By default, this cache is set to a maximum of 100 entries.             |
| `caching.inter_query_builtin_value_cache.named.regex.disabled`           | `bool`  | No       | Explicitly disable `regex`, by default this is `false`. Setting this to `true` will disable `regex`.                                                                                                                                                                                               |
| `caching.inter_query_builtin_value_cache.named.glob.max_num_entries`     | `int`   | No       | Maximum number of entries in the `glob` cache, used by the [`glob` built-in functions](./policy-reference/builtins/glob) to cache compiled patterns. OPA will drop random items from the cache if this limit is exceeded. By default, this cache is set to a maximum of 100 entries.                |
| `caching.inter_query_builtin_value_cache.named.glob.disabled`            | `bool`  | No       | Explicitly disable `glob`, by default this is `false`. Setting this to `true` will disable `glob`.                                                                                                                                                                                                 |
