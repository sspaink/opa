...
| `[_].trace_id`                     | `string`        | Unique identifier of a trace generated for each incoming request for traceability. This is a hex string representation compliant with the W3C trace-context specification. See more at the [W3C trace-context specification](https://www.w3.org/TR/trace-context/#trace-id). Only present when [distributed tracing](./configuration/#distributed-tracing) is enabled.                                  |
| `[_].span_id`                      | `string`        | Unique identifier of a span in a trace to assist traceability. This is a hex string representation compliant with the W3C trace-context specification. See more at the [W3C trace-context specification](https://www.w3.org/TR/trace-context/#parent-id). Only present when [distributed tracing](./configuration/#distributed-tracing) is enabled.                                                     |
...
:::info
`trace_id` and `span_id` are only recorded when [distributed tracing](./configuration/#distributed-tracing) is
configured. Without `distributed_tracing.type` OPA creates no spans, so there is no trace context to log and the two
fields are omitted from the event, even if the request carried a `traceparent` header.

When distributed tracing is configured, OPA continues the trace described by an incoming
[W3C `traceparent`](https://www.w3.org/TR/trace-context/) header, so `trace_id` matches the caller's trace. See
[Monitoring](./monitoring/#correlating-decision-logs-with-traces) for how to record these fields without exporting
